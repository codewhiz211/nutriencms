...
@Component({
  selector: 'app-sold-wool',
  templateUrl: './sold-wool.component.html',
  styleUrls: ['./sold-wool.component.scss']
})
export class SoldWoolComponent implements OnInit {
  ...
  async getDataSource(params: any) {
    const response: any = await this.woolSearchService.getSoldWoolProcessData(params).toPromise();
  ...
  getExcelData() {
    ...
    this.apiESaleyardService.postGetFile('awhwool/ExportToExcelSoldWool', bodyData, 'blob')
      .subscribe(
        (resultBlob: Blob) => {
