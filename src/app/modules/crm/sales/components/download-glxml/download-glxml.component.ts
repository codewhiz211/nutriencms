...
@Component({
  selector: 'app-download-glxml',
  templateUrl: './download-glxml.component.html',
  styleUrls: ['./download-glxml.component.scss']
})
export class DownloadGlxmlComponent implements OnInit {
  ...
  async getGLxmlData(params: any) {
    const response: any = await this.api.post('report/GetSaleGLxml',params).toPromise();
    ...
    switch (event.action) {
      case 'Filter_Header':
        ...
        case 'Filter_Click':
          ...
          filter = {
            GridConditions: [],
            DataField: event.colData.objectKey === 'Salenumber'?'sales.dmocrmheaderinfsaleid':event.colData.objectKey,
