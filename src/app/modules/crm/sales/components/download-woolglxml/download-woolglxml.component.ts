...
@Component({
  selector: 'app-download-woolglxml',
  templateUrl: './download-woolglxml.component.html',
  styleUrls: ['./download-woolglxml.component.scss']
})
export class DownloadWoolglxmlComponent implements OnInit {
    ...
    config: {
      header: {
        columns: [
          ...
          {
            objectKey: 'SaleCentre',
            displayName: 'Sale Centre',
            width: '14%'
          },
          {
           ...
           {
            objectKey: 'StorageCenter',
            displayName: 'Storage Center',
            width: '14%'
          },
          {
  ...
  async getGLxmlData(params: any) {
    const response: any = await this.api.post('report/getWoolGLxmlData',params).toPromise();
    ...
    switch (event.action) {
      case 'Filter_Header':
        ...
        case 'Filter_Click':
          ...
          filter = {
            GridConditions: [],
            DataField: event.colData.objectKey === 'DocumentNumber'?'DocumentNumber':event.colData.objectKey,
